...
import numpy as np
#from utilsconvformer.visualization import attentionheatmap_visual, attentionheatmap_visual2
...
class CNNAttention(nn.Module):
    ...
    def forward(self, x, mode="train", smooth=1e-4):
        ...
        dis = F.interpolate(dis.unsqueeze(0), size=(64, 64), mode='bilinear').squeeze(0)

        #print(f"attn shape: {attn.shape}")  # 例如: (batch_size, heads, height, 64)
        #print(f"dis shape: {dis.shape}")  # 例如: (1, height, 256, 256)
...
class Setr_ConvFormer(nn.Module):
    def __init__(self, n_channels, n_classes, imgsize, patch_num=16, dim=512, depth=12, heads=8, mlp_dim=512*4, dim_head=64, dropout=0.1, emb_dropout=0.1):
        super().__init__()
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv2d(self.dmodel // 4, dim, kernel_size=1),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
        )
    ...
    def forward(self, img):
        ...
        # encoder
        x = self.transformer(x)  # b c h w -> b c
        x = self.decoder(x)
        return x
    def infere(self, img):
